...
#### Numero di partecipanti e "talento" della squadra (Gigante, Femminile)
...
#| fig-height: 5
library(MASS)
ggplot(data = gigante_freq, aes(x = Partecipanti, y = Graduatoria, label = Ateneo))  + theme_light() + xlab("Numero di partecipanti") + ylab("Punteggio finale") + geom_smooth(method = "rlm", se = F, linetype = "dotted", size = 0.6) + geom_text(size = 3, vjust = "top", check_overlap = TRUE)+ scale_x_continuous(n.breaks = 10)
```
...
```{r}
contributo <- residuals(rlm(Graduatoria ~ Partecipanti, data = gigante_freq))
names(contributo) <- gigante_freq$Ateneo
...
<!-- ```{r} -->
<!-- m2 <- lm(Graduatoria ~ Partecipanti + Tempo_migliori_4, data = gigante_freq) -->
<!-- summary(m2) -->
<!-- ``` -->
