...
## Generalization of the predictive approach
...
- Taking a [predictive approach]{.blue}, one can [characterize the prior by specifying the sequence of predictive distributions]{.blue}.
...
- The possibility of specifying a sequence of one-step-ahead predictive distributions is appealing!
  ...
  - $\rightarrow$ explicitly connects prediction and inference

. . .

- The predictive construction of the DP prior is a Pòlya-urn scheme for which $Y_1\sim P_0$ and $Y_{n+1}\mid y_{1:n}\sim P_n$ for $n\ge 1$, where
$$
P_n(A) = \mathbb{P}\{Y_{n+1}\in A\mid y_{1:n}\}
= \frac{\alpha}{\alpha+n} P_0(A) + \frac{1}{\alpha+n}\sum_{i=1}^n\mathbb{1}(y_i\in A)
$$
...
![](img/pl_ill_slide.pdf){fig-align="center" width="100%"}
...
![](img/ker_plot_slide.pdf){fig-align="center" width="100%"}
...
![](img/pl_cov_sim.pdf){fig-align="center" width="85%"}

## Application

- We analyze the occurrence rates of 4 species corvids in Finland in year 2009 across different temperatures and habitats.

![](img/pl_appl_slide_1.pdf){fig-align="center" width="65%"}

## Application II

![](img/pl_appl_slide_2.pdf){fig-align="center" width="85%"}
...
::: {.callout-warning title="De Finetti's representation theorem"}
Let $(Y_n)_{n\ge 1}$, $Y_n\in\mathcal{Y}$, be a sequence of exchangeable random variables with probability law $P$.
Then there exists a unique probability measure $\Pi$ such that, for any $n\ge 1$,
...
![Illustrative example. Hellinger distance of MAD-ada (red), MAD-1 (blue), and DP (black) predictive distributions with respect to the density of the true data generator.](img/emp_cons.pdf){fig-align="center" width="85%"}
...
![](img/cor_var_plot.pdf){fig-align="center" width="85%"}
