...
The predictive allocation probabilities for a generic subject $i$ under the proposed and a na\"ive model in which it is assumed that there are no unseen traits, with $N = k$, satisfy the inequality:
$$
...
- We iteratively sample the labels of $\mu_i \mid \bm Z, \bm{\mu}_{-i}$ for $i=1,\dots,n$ according to the [full conditional probabilities]{.orange} $p_{iq}$ and $p_{i,\text{new}}$, which are proportional to
$$
...
- This updating scheme is particularly straightforward thanks to the closed-form expression of $\pi_n(\bm a; \lambda, \psi)$.
